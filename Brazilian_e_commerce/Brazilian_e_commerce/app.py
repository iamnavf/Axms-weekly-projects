...
def sales():
    ...
    top_state = (df.groupby("customer_state")["price"].sum().idxmax())

    return render_template("sales.html",total_revenue=total_revenue,avg_order_value=avg_order_value,
                           top_category=top_category,top_state=top_state)
...
def customer():
    ...
    insight_4 = ("Average customer spending is " f"{avg_customer_spending:,.2f}.")
...
def product():
    ...
    insight_3 = f"The average product price is {avg_product_price:,.2f}."
...
def payment_review():
    ...
    insight_2 = (f"The average payment value is  {avg_payment_value:,.2f}.")
